...
export class PermissionsInterceptor implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler) {
    ...
    var userid = request.user.id as number ; 
    
    if(!userid) throw new UnauthorizedException('Login again'); 
    
    var permissionsofUser = await this.usersRepository.findUserPermissions(userid); 

    if(permissionsofUser?.roles == null ) throw new UnauthorizedException('No permissions exists for the user'); 
